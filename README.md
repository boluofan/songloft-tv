...
- **设置**：主题模式（深/浅/暗夜/跟随系统）、主题色调（黛青蓝/薄荷绿/珊瑚粉/蜜橘橙）、音质选择、播放缓存、后台播放、音效、睡眠定时器、歌词样式、日志导出（网页下载）、应用更新检查、问题反馈、操作说明、关于
- **遥控体验**：全局 D-Pad 焦点导航、默认焦点落在底部 Tab 便于快速切换、退出二次确认、危险操作二次确认、三段式返回（回顶 → 聚焦顶部按钮 → 底部 Tab → 回首页）
